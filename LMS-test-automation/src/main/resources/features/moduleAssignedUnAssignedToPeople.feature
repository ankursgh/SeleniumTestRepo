#Author: navaraj@carriersedge.com
  ...
  #Role - Customer Support: Programs >> Module >> Show Modules >> People Assigned/unassigned , Test-LMST-717
	
	# Bug commented in steps 8 and 9, 
  ...
  Scenario: Assigning and Unassigning modules to users when logged in as Customer Support.
   ...
   # And I checked on the `Include login details in the notification`.
    #Then I waited for 2 seconds.
     ...
     #Step 8, and 9 for bulk users, More than 600 users
		#No any notification saying "You are about to reset the status for X users. Are you sure want to continue?"
		#We have a ticket for LMS 1837
          
    # Expected pop up message "You are about to reset the status for X users. Are you sure want to continue?" but due to bug, the pop up does not appear
    #Then I saw a pop up message-"You are about to reset the status for X users. Are you sure want to continue?".
    ...
    Then I waited for 4 seconds.
     ...
     Then I waited for 6 seconds.
     #Then I waited for 2 seconds.
     #And I verified that 703 people assigned to the "Winter Driving" module which id is "module-250" on the Show Modules page.          
    ...
    #Step 13 and 14 already verified in above step
    # Step 15
    # Has to implemented
    # Step 16 , Saving without change will no change anything effect below
    When I clicked on `x people assigned` to module "Winter Driving" module which id is "module-250".
    #Then I waited for 2 seconds.
     And clicked on the save button.
     ...
     # Step 17, Below Are the steps for unassigning a course.
     # Steps 18, login as a Customer support user, Which is already 
     # Steps 19, 
     #Go to Programs >> Modules >> Show Modules >> Choose any module , which is done already in the above step and for guided text below
     When I hovered Over on the `x people assigned` of the module "Winter Driving" module which id is "module-250".
     Then I waited for 1 seconds.
